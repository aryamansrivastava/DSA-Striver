...
int maximumWidth(Node *root) {
    ...
    while(!q.empty()) {
        int size = q.size();
        int first, last;  // Store the first and last positions of nodes in the current level
        for (int i = 0; i < size; i++)
        {
            ...
            if(node->right) q.push({node->right, 2*(ind-1)+1});
        }
...
int main(){
    ...
    cout << "Maximum Width of Binary Tree3: " << maximumWidth(root) << endl;
    return 0;
}